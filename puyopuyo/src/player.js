class Player {
    ...
    static playing(frame) {
        ...
        if (this.keyStatus.right || this.keyStatus.left) {
            ...
            if (canRotate) {
                ...
                // 次の状態を先に設定しておく
                this.puyoStatus.x += cx;
                const distRotation = (this.puyoStatus.rotation + 270) % 360;
                const dCombi = [[1, 0], [0, -1], [-1, 0], [0, 1]][distRotation / 90];
                this.puyoStatus.dx = dCombi[0];
                this.puyoStatus.dy = dCombi[1];
                return 'rotating';
    ...
    static rotating(frame) {
        ...
        this.puyoStatus.rotation = this.rotateFromRotation + (this.left_rotation_flg ? ratio * 90 : -ratio * 90);
        this.setPuyoPosition();

        if (ratio === 1) {
            this.puyoStatus.rotation = (this.left_rotation_flg ? (this.rotateFromRotation + 90) : (this.rotateFromRotation - 90 + 360)) % 360;
            return false;
        }
        return true;
    }
